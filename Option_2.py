...
while True:
    ...
    if min_length > 0:
        ...
        cv2.fillPoly(overlay, [quad_points], (0, 150, 0))
        alpha = 0.2
        cv2.addWeighted(overlay, alpha, birdseye, 1 - alpha, 0, birdseye)

        # Inverse transform
        inv_matrix = cv2.getPerspectiveTransform(pts2, pts1)
        original_perpective_lane_image = cv2.warpPerspective(birdseye, inv_matrix, (640, 480))
        result = cv2.addWeighted(frame, 1, original_perpective_lane_image, 0.5, 0)

        # Vẽ đường và thông số
        line_length = 100
        end_x = int(320 + line_length * np.sin(np.radians(steering_angle)))
        end_y = int(480 - line_length * np.cos(np.radians(steering_angle)))
        cv2.line(result, (320, 480), (end_x, end_y), (0, 255, 0), 2)
